...
CREATE TABLE `__new_test_sheet_points` (
	...
	CONSTRAINT "test_sheet_points_settlement" CHECK("__new_test_sheet_points"."settlement" is null or "__new_test_sheet_points"."settlement" in ('holds', 'broken', 'human'))
);
--> statement-breakpoint
INSERT INTO `__new_test_sheet_points`("id", "report_id", "position", "criterion_id", "text", "verdict", "comment", "settlement", "settlement_note") SELECT "id", "report_id", "position", "criterion_id", "text", "verdict", "comment", NULL, NULL FROM `test_sheet_points`;--> statement-breakpoint
