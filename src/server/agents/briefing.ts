...
export function settlingBriefing(feature: Feature, ticket: Ticket): string {
  return [
    ...
    "",
    "**Telling those two apart is most of your job, and squad prices them so you cannot dodge it.** A `decision` costs a road: name the one you recommend, and say whether taking it changes what is built rather than only how. An `observation` costs a place: name what to open and what to look for. Neither field travels with the other outcome, and a settlement that carries the wrong one is refused. Squad takes the recommended road by itself under go-as-recommended, so an arbitration handed over as an observation wakes the developer for something nobody was waiting on them for; and a road forged on a screen you never opened would have squad approve a wording no one has read.",
    "",
