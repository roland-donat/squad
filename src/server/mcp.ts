    ...
    .array(
      z.object({
        ...
        lookAt: z
          ...
          .optional()
          .describe(
            `On an \`observation\`, and there only: what to open, and what to look for once it is open. Required, and refused on any other outcome. It is the whole of what the developer has to go on, so name the screen, the file or the command that puts it in front of them, not the subject. ${inlineMarkdown}`,
          ),
      }),
    )
    .describe(
...
function buildMcpServer({
  ...
  server.registerTool(
    squadTools.settleSheet,
    {
      ...
      description:
        "Answers every point of the test sheet you were opened for: what you ran and what it said. Only the points you declare `observation` reach the developer; a point that holds is checked off, a point that is broken goes back to the sub-session with your evidence, and a `decision` is taken by squad under go-as-recommended along the road you name. A sheet you never answer through this tool reaches the developer untouched, so giving up costs nothing but never pretend a point holds without having run something.",
